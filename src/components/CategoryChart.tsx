...
const CategoryChart = ({monthlyTransactions,isLoading}:CategoryChartProps) => {
  ...
  const options = {
    ...
    const getCateforyColor = (category: IncomeCategory | ExpenseCategory):string => {
    ...
    datasets: [
      {
        ...
        backgroundColor: categoryLabels.map((category) => {
          return getCateforyColor(category)}),
  ...
  return(
    <>
      ...
      <Box  sx={{flexGrow:1, display:'flex', alignItems:'center', justifyContent:'center'}} >
        { isLoading ? (<CircularProgress />) : monthlyTransactions.length > 0 ? (<Pie data={data} options={options}  />) : (<Typography>データがありません</Typography>)  }
